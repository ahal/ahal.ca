...
def display_article(request, year=None, slug=None, draft=False, template='articles/article_detail.html'):
    """Displays a single article."""

    try:
        if not year or not slug:
            article = Article.objects.live(user=request.user, draft=draft).order_by('-publish_date')[0]
        else:
            article = Article.objects.live(user=request.user, draft=draft).get(publish_date__year=year, slug=slug)
